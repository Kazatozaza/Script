...
-- Move to the monster and face it (Behind monster and face towards it)
    ...
    for _, obj in pairs(game:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and obj.Name == monsterName then
            ...
            if monsterHRP then
                ...
                local offset = Vector3.new(0, 5, -1)  -- Behind the monster
...
-- Auto-attack function with 0 milliseconds delay
function autoAttackMonster(monsterName)
    ...
    if tool then
        ...
        -- การตั้งค่าการโจมตีอัตโนมัติ
        task.spawn(function()
            while autoFarming do
                for _, obj in pairs(game:GetDescendants()) do
                    if obj:IsA("Model") and obj.Name == monsterName and obj:FindFirstChild("Humanoid") then
                        -- ใกล้เคียงมอนสเตอร์ก็จะโจมตี
                        local monsterHRP = obj:FindFirstChild("HumanoidRootPart")
                        if monsterHRP then
                            -- Move behind the monster and face it
                            if moveToMonster(monsterName) then
                                -- การโจมตีอัตโนมัติเมื่อเจอเป้าหมาย
                                tool:Activate()
                            end
                            break
                        end
                ...
                task.wait(0)  -- แทนการหน่วงเวลาที่ 0 milliseconds
            end
        end)
    end
end
...
toggleButton.Position = UDim2.new(0.5, -75, 0, 10) -- Positioned at the top center of the screen
...
toggleButton.MouseButton1Click:Connect(function()
    mainFrame.Visible = not mainFrame.Visible  -- Toggle the visibility of the main frame
end)
